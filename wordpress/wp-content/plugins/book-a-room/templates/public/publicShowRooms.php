...
$bookaroom_public = new bookaroom_public;
?>
	...
	if ( true != <?php echo $showContract; ?> ) {
		jQuery( document ).ready( function () {
			jQuery( 'a.reservelink' ).click( function ( ev ) {
				...
				jQuery.Zebra_Dialog( <?php echo json_encode( $bookaroom_public->replace_checks( get_option( 'bookaroom_content_contract' ) ) ); ?>, {
					'type': 'question',
					'width': realWidth,
					...
					'buttons': [
                                    {
                                        caption: 'Accept',
                                        callback: function () {
                                            var $all_checkboxes = jQuery('input[name="bookaroom_secure"]').length;
                                            var $checked_checkboxes = jQuery('input[name="bookaroom_secure"]:Checked').length;
                                            if( $all_checkboxes > 0 && $all_checkboxes !== $checked_checkboxes ) {
                                                new jQuery.Zebra_Dialog("You must confrim that you agree by checking each box.", {
                                                    auto_close: 2000,
                                                    buttons: false,
                                                    modal: false,
                                                    type: "error"
