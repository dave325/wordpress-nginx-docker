<?php
add_action('widgets_init', 'htevent_twitter_sidebar_load_widgets');

function htevent_twitter_sidebar_load_widgets()
{
	register_widget('htevent_twitter_sidebar_Widget');
}

if(!class_exists('htevent_twitter_sidebar_Widget')){
	class htevent_twitter_sidebar_Widget extends WP_Widget {

		function __construct(){
			$widgetOps = array('classname' => 'htevent-twitter-widget', 'description' => esc_html__('Display latest tweets', 'htevent'));
			parent::__construct('htevent_twitter_sidebar', esc_html__('Sidebar - Twitter', 'htevent'), $widgetOps);
		}
				...
				if( !isset($tweets->errors) && is_array($tweets) ){
					...
					foreach( $tweets as $tweet ){
						...
						?>
							...
							<div class="single-widget sb-twitter-feed">
								<ul>
								
									<li>
										<a href="<?php echo esc_url($user_link); ?>" target="_blank"><?php echo esc_html($tweet->user->name); ?></a>
										
										<p> <?php echo wp_trim_words( $tweet->text, 6, ' ' ); ?></p>
										
										<a class="time" href="https://twitter.com/">										
											<?php 
											if( $relative_time ){
		...
		?>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('relative_time'); ?>"><?php esc_html_e('Relative time', 'htevent'); ?></label>
			</p>
			<p>
